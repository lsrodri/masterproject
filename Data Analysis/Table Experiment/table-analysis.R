...
data <- read.csv("table-results.csv")
...
accuracies  <- data.frame(pub=character(), condition=character(), estimate=numeric(), ci.low=numeric(), ci.high=numeric(), order=integer())

ord <- 6 # adjust this number to the number of values from previous work that are being added further below

cis <- bootstrapCI(subset(data, condition == "BarChart")$accuracy)
accuracies  <- rbind(accuracies, data.frame(pub="", condition="BarChart", estimate=cis[1], ci.low=cis[2], ci.high=cis[3], order=inc(ord)))

cis <- bootstrapCI(subset(data, condition == "Length" )$accuracy)
accuracies  <- rbind(accuracies, data.frame(pub="", condition="Length", estimate=cis[1], ci.low=cis[2], ci.high=cis[3], order=inc(ord)))

cis <- bootstrapCI(subset(data, condition == "Area")$accuracy)
accuracies  <- rbind(accuracies, data.frame(pub="", condition="Area", estimate=cis[1], ci.low=cis[2], ci.high=cis[3], order=inc(ord)))
...
prevD  <- data.frame(pub="Cleveland [1984]", condition="Type I\n(BarChart)", estimate=2.4200, ci.low=2.1979, ci.high=2.6644, order=inc(ord))
...
ggplot(data=cidf) +
  ...
  geom_pointrange(aes(y=estimate, x=reorder(condition, order), ymin=ci.low, ymax=ci.high), size=0.7) +
...
ggsave("table_accuracy.pdf", width = 8, height = 2)
...
coeffs <- ddply(data, .(condition, participant), function(dat){
  ...
  data.frame(participant=dat$participant[1], condition=dat$condition[1], coef = a)
})
...
# layout graphs in 1 row, 3 columns
pdf("table - psychophysical functions per participant.pdf", width = 12, height = 3)
...
# horizontal stimuli
## BarChart
curve((x), xlim=c(0,100), ylim=c(0,100), col="red", ylab="predicted estimated ratio (in percent)", xlab="actual ratio (in percent)", main="Bar Chart")
d_ply(subset(data, condition=="BarChart"), .(participant), function(d){
...
text(x=c(20,80), y=c(70, 30), labels = c(paste("min: ", prettyNum(min(subset(data, condition=="BarChart")$coef), digits=2)), paste("max: ", prettyNum(max(subset(data, condition=="BarChart")$coef), digits=2))))

## Length
curve((x), xlim=c(0,100), ylim=c(0,100), col="red", ylab="predicted estimated ratio (in percent)", xlab="actual ratio (in percent)", main="Length")
d_ply(subset(data, condition=="Length"), .(participant), function(d){
  curve(regFun(x,a=d$coef[1]), add = TRUE, col=rgb(0,0,0,0.5))
  #p  <- p + geom_smooth(data=d, aes(x = d$trueP, y = d$estimate), method="loess", alpha=I(0.1))
})
text(x=c(20,80), y=c(70, 30), labels = c(paste("min: ", prettyNum(min(subset(data, condition=="Length")$coef), digits=2)), paste("max: ", prettyNum(max(subset(data, condition=="Length")$coef), digits=2))))

## Area
curve((x), xlim=c(0,100), ylim=c(0,100), col="red", ylab="predicted estimated ratio (in percent)", xlab="actual ratio (in percent)", main="Area")
d_ply(subset(data, condition=="Area"), .(participant), function(d){
  curve(regFun(x,a=d$coef[1]), add = TRUE, col=rgb(0,0,0,0.5))
  #p  <- p + geom_smooth(data=d, aes(x = d$trueP, y = d$estimate), method="loess", alpha=I(0.1))
})
text(x=c(20,80), y=c(70, 30), labels = c(paste("min: ", prettyNum(min(subset(data, condition=="Area")$coef), digits=2)), paste("max: ", prettyNum(max(subset(data, condition=="Area")$coef), digits=2))))
